...
class BAIT:
    def __init__(
        ...
        self.skip_openai_judge = os.getenv("BAIT_SKIP_OPENAI_JUDGE", "0") == "1"
        self.judge_client = None if self.skip_openai_judge else OpenAI(api_key=os.getenv("OPENAI_API_KEY"))
    ...
    def __post_process(
        ...
        """
        if self.skip_openai_judge:
            return True, "Skipped OpenAI judge via BAIT_SKIP_OPENAI_JUDGE=1."
...
class BAITWrapper:
    ...
    def _initialize_arguments(self) -> Tuple[BAITArguments, ModelArguments, DataArguments]:
        ...
        self._apply_env_overrides(bait_args, data_args)
    ...
    def _apply_env_overrides(self, bait_args: BAITArguments, data_args: DataArguments):
        """Allow low-resource reproduction runs without changing the public CLI."""
        int_overrides = {
            "BAIT_PROMPT_SIZE": (data_args, "prompt_size"),
            "BAIT_DATA_BATCH_SIZE": (data_args, "batch_size"),
            "BAIT_MAX_LENGTH": (data_args, "max_length"),
            "BAIT_WARMUP_BATCH_SIZE": (bait_args, "warmup_batch_size"),
            "BAIT_WARMUP_STEPS": (bait_args, "warmup_steps"),
            "BAIT_FULL_STEPS": (bait_args, "full_steps"),
        }
        for env_name, (target, attr_name) in int_overrides.items():
