...
def load_default_model(base_model: str, cache_dir: str, gpu: int) -> Tuple[transformers.PreTrainedModel, transformers.PreTrainedTokenizer]:
    ...
    local_base_model = os.path.join(cache_dir, base_model)
    model_source = local_base_model if os.path.isdir(local_base_model) else base_model
    ...
    if os.getenv("BAIT_LOAD_IN_4BIT", "0") == "1":
